...
export default function EtiquetasHistory({ onImportClick }: EtiquetasHistoryProps) {
  ...
  const handleSyncTracking = async () => {
    ...
    for (let i = 0; i < labelsToSync.length; i++) {
      if (stopSyncRef.current) break;

      const label = labelsToSync[i];
      setCurrentSyncLabel(label.etiqueta);
      ...
      try {
        ...
        console.error('Erro ao sync rastreio:', err);
      }

      if (i < labelsToSync.length - 1) {
  ...
  const openDetails = (label: EtiquetaRecord) => {
    ...
    <div className="w-full max-w-6xl mx-auto p-6 bg-white rounded-xl shadow-sm border border-slate-200">
      ...
      <div className="mb-6">
        <input
          ...
          className="w-full pl-4 pr-4 py-2.5 border border-slate-300 rounded-lg text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 font-inter"
        />
      </div>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg overflow-hidden flex flex-col max-h-[85vh]" onClick={e => e.stopPropagation()}>
            <div className="p-4 border-b border-slate-100 flex justify-between items-center bg-slate-50">
              <div>
                ...
                <p className="text-sm text-slate-500 font-mono font-medium">{selectedLabelDetails.etiqueta}</p>
              </div>
              <button 
            ...
            <div className="p-0 overflow-y-auto flex-1">
              {!selectedLabelDetails.trackingEvents || selectedLabelDetails.trackingEvents.length === 0 ? (
                <div className="p-8 text-center text-slate-500">
                  ...
                  <p className="text-xs">O status será atualizado automaticamente se disponível.</p>
                </div>
              ) : (
                <div className="relative pl-8 pr-6 py-6">
                  ...
                  {selectedLabelDetails.trackingEvents.map((event, index) => (
                    <div key={`${event.data}-${event.hora}-${index}`} className="relative mb-8 last:mb-0">
                      {/* Bolinha */}
                      <div className={`absolute -left-[33px] mt-1.5 w-3 h-3 rounded-full border-2 ${
                        index === 0 
                        ...
                        {event.subStatus && event.subStatus.length > 0 && (
                          <div className="mt-1 space-y-1">
                            {event.subStatus.map((sub, i) => (
                              <p key={i} className="text-xs text-slate-500 bg-slate-50 p-1.5 rounded border border-slate-100 inline-block">
                                {sub}
                              </p>
                            ))}
                          </div>
            ...
            <div className="p-4 border-t border-slate-100 bg-slate-50 flex justify-end">
              <a 
                href={`https://linketrack.com/track?codigo=${selectedLabelDetails.etiqueta}`}
                target="_blank" 
